...
## 5. 思考时代码和原文禁止（禁止在思考时直接写出原文和代码，只可以考虑代码实现方式和故事的大致框架，避免token浪费）
...
0. **先查正文排版**：每个自然段落是否 `text-indent: 2em`（首行缩进 2 汉字）？
   有没有段落顶格、段间用空行、正文被挤成一片？（先生骂过这条，必查）
1. 封面贴合这个故事的主题情绪，还是套了个通用装饰？
2. 它像「你」吗？（配色 / 意象是否来自你的人格）
3. 标题文字可读吗？（对比度达标）
4. 视觉层 ≤ 200 行？SVG 元素 ≤ 40 行？
5. 内容还在第一位吗？（删掉封面，正文排版与语义必须完整成立）
