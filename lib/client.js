...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function openAestheticsViewer() {
      ...
      fHelp.textContent = '点「选择字体来源」后，在同一个系统窗口里浏览文件夹和字体文件：选字体文件后点「挂载字体」；进入目标目录后点「挂载当前文件夹」。也可手动填写路径。'
      ...
      pickBtn.addEventListener('click', function () {
        ...
        fontStatusDiv.textContent = '正在打开字体来源选择器…'
        aesWithTimeout(aesRpc('pick-font-source'), 5 * 60 * 1000, '字体来源选择器等待超时').then(function (d) {
          if (d && d.error) {
            resetBtn(pickBtn)
          ...
          return loadFontsOnly().then(function () {
            ...
            fontStatusDiv.textContent = '已挂载：' + ((d && d.path) || '')
          })
        }).catch(function (e) {
          resetBtn(pickBtn)
