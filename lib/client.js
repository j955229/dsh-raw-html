...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function openAestheticsViewer() {
      ...
      fontStatusDiv.style.marginTop = '4px'
      foot.appendChild(fontStatusDiv)
      function updateRoots() {
        rootsDiv.textContent = data.extraRoots && data.extraRoots.length
          ? '已挂载：' + data.extraRoots.join(' · ')
          : '已挂载：无（内置字体仍可直接使用）'
        ...
        var names = external.slice(0, 6).map(function (f) { return aesDispFont(f.family || aesBaseName(f.name || '')) }).filter(Boolean)
      ...
      pickBtn.addEventListener('click', function () {
        ...
        aesRpc('pick-fonts-root').then(function (d) {
