    ...
    function aesBuildStyleSection(st, fontMap, aliasMap, installed, lockedSlug) {
      ...
      if (names.length) {
        ...
        for (var i = 0; i < names.length; i++) {
          ...
          opt.value = inst ? inst.rel : (aliasRel || '')
          opt.setAttribute('data-family', n)
          opt.textContent = aesDispFont(n) + (available ? ' ✓' : ' · 未安装')
        ...
        prev.textContent = '春风又绿江南岸 · 字体预览 0123456789 Aa'
    ...
    function openAesNewStyle(existing) {
      ...
      aesRpc('list-fonts').then(function (fd) {
        ...
        sub.textContent = existing
          ? '只编辑显示名称、色板与后续生成使用的候选字体'
          : '设置名称、色板与候选字体；标识与内部元数据自动处理'
        tWrap.appendChild(t)
        tWrap.appendChild(sub)
        var close = document.createElement('button')
        close.className = 'aes-btn'
