...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function openAestheticsViewer() {
      ...
      fHelp.textContent = '可直接选择字体文件夹或单个字体文件；也可手动填写路径。文件夹选择器只显示文件夹，单个 .ttf/.otf 请用「选择字体文件」。'
      ...
      pickBtn.title = '打开系统文件夹选择器并挂载所选目录'
      var pickFileBtn = document.createElement('button')
      pickFileBtn.className = 'aes-btn'
      pickFileBtn.textContent = '🔤 选择字体文件'
      pickFileBtn.title = '直接选择 .ttf / .otf / .woff / .woff2 / .ttc 字体文件'
      ...
      pickBtn.addEventListener('click', function () {
        ...
        setBtnState(pickBtn, '选择中…', true)
        fontStatusDiv.textContent = '正在打开系统文件夹选择器…'

        var desktopPicker = window.__DSH_DESKTOP_PICK_DIRECTORY__
        var pickTask
        if (typeof desktopPicker === 'function') {
          pickTask = window.Promise.resolve().then(function () { return desktopPicker() }).then(function (picked) {
            if (!picked) return { canceled: true }
            return aesRpc('add-fonts-root', { path: picked }).then(function (d) {
              if (d && !d.error) d.path = picked
              return d
            })
          })
        } else {
          // 非 DSH Desktop 或 bridge 不存在时，再尝试 Harness directoryPicker capability。
          pickTask = aesRpc('pick-fonts-root')
        }

        pickTask.then(function (d) {
          if (d && d.error) {
            resetBtn(pickBtn)
            fontStatusDiv.textContent = '选择失败：' + (d.error.message || JSON.stringify(d.error))
            return
          }
          if (d && d.canceled) {
            resetBtn(pickBtn)
            fontStatusDiv.textContent = '已取消选择'
            return
          }
          data.extraRoots = (d && d.extraFonts) || data.extraRoots
          return loadFontsOnly().then(function () {
            resetBtn(pickBtn, '已挂载 ✓')
            fontStatusDiv.textContent = '已挂载文件夹：' + ((d && d.path) || '')
          })
        }).catch(function (e) {
          resetBtn(pickBtn)
          fontStatusDiv.textContent = '选择失败：' + ((e && e.message) || e)
        })
      })

      pickFileBtn.addEventListener('click', function () {
        if (pickFileBtn.disabled) return
        setBtnState(pickFileBtn, '选择中…', true)
        fontStatusDiv.textContent = '正在打开字体文件选择器…'
        aesRpc('pick-font-file').then(function (d) {
          if (d && d.error) {
            resetBtn(pickFileBtn)
            fontStatusDiv.textContent = '选择失败：' + (d.error.message || JSON.stringify(d.error))
            return
          }
          if (d && d.canceled) {
          ...
          data.extraRoots = (d && d.extraFonts) || data.extraRoots
          return loadFontsOnly().then(function () {
            resetBtn(pickFileBtn, '已挂载 ✓')
            fontStatusDiv.textContent = '已挂载字体：' + ((d && d.path) || '')
          })
        }).catch(function (e) {
          resetBtn(pickFileBtn)
          fontStatusDiv.textContent = '选择失败：' + ((e && e.message) || e)
