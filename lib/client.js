  ...
  factory: function (require) {
    ...
    function aesApplyTheme(view) {
      ...
      var vars = dark ? {
        '--aes-panel': '#20242a',
        '--aes-card': '#292f36',
        '--aes-card-hover': '#313943',
        '--aes-input': '#181c21',
        '--aes-control': '#2c333b',
        '--aes-text': '#f4f6f8',
        '--aes-muted': '#b9c1cb',
        '--aes-border': '#56616f',
        '--aes-border-strong': '#8a97a8',
        '--aes-accent': '#78aefc'
      } : {
        '--aes-panel': '#f4f6fa',
        '--aes-card': '#ffffff',
        '--aes-card-hover': '#f6f8fb',
        '--aes-input': '#ffffff',
        '--aes-control': '#eef1f5',
        '--aes-text': '#171a1f',
        '--aes-muted': '#596273',
        '--aes-border': '#c4cad3',
        '--aes-border-strong': '#7f8998',
        '--aes-accent': '#245fa8'
      }
      for (var key in vars) view.style.setProperty(key, vars[key])
      view.style.colorScheme = dark ? 'dark' : 'light'
      view.setAttribute('data-aes-theme', dark ? 'dark' : 'light')
    }
    ...
    function aesBuildStyleSection(st, fontMap, aliasMap, installed, lockedSlug) {
      ...
      if (names.length) {
        ...
        opt0.value = ''
        opt0.textContent = '字体预览（仅预览）'
        opt0.title = '仅预览字体效果，不会修改风格，也不会替换已有 VCP'
    ...
    function aesFontCategory(label, initial, aliasMap, installed) {
      ...
      function buildMenu(filterText) {
        ...
        aliasKeys.forEach(function (ak) {
          ...
          meta.textContent = installed[ak] ? '可用 ✓' : '未安装'
          item.appendChild(meta)
          if (!installed[ak]) {
            item.disabled = true
            item.style.opacity = '0.45'
            item.style.cursor = 'not-allowed'
            item.title = '字体未安装：请先在美学系统底部挂载字体库并重新扫描'
          }
          if (fontLazyIO && installed[ak]) fontLazyIO.observe(item)
          if (installed[ak]) item.addEventListener('click', function () {
            selected.push(ak)
      ...
      function renderChips() {
        ...
        selected.forEach(function (ak) {
          ...
          chip.appendChild(document.createTextNode(aesDispFont(ak) + (isAliasKey(ak) && !installed[ak] ? '（未装）' : '')))
    ...
    function openAesNewStyle(existing) {
      ...
      aesRpc('list-fonts').then(function (fd) {
        ...
        var sub = document.createElement('div')
        sub.className = 'aes-subtitle'
        sub.textContent = existing ? '修改风格定义；保存后影响后续生成，不会修改已有 VCP' : '把找到的关键词与想法，变成一份可被检索的风格'
        tWrap.appendChild(t)
        tWrap.appendChild(sub)
        var close = document.createElement('button')
        close.className = 'aes-btn'
        ...
        fontHint.textContent = '这里只设置后续生成的候选字体，不会修改已有 VCP；每类可多选，agent 会按内容选择。'
