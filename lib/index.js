...
/** 打开统一的字体来源选择器。
 * Windows 使用一个 OpenFileDialog 同时展示文件夹与字体文件：
 * - 双击文件夹进入；
 * - 选中字体文件后打开 = 挂载该文件；
 * - 进入目标文件夹后保持“选择当前文件夹”并打开 = 挂载当前文件夹。
 * 这样用户不需要先决定“选文件”还是“选目录”。
 */
function pickNativeFontSource() {
  if (process.platform === 'win32') {
    const script = [
      ...
      "$d.Title = '选择字体来源：可选择字体文件，或进入目标文件夹后直接点“打开”挂载整个文件夹'",
      "$d.Filter = '字体文件 (*.ttf;*.otf;*.woff;*.woff2;*.ttc)|*.ttf;*.otf;*.woff;*.woff2;*.ttc|所有文件 (*.*)|*.*'",
      "$d.Multiselect = $false",
      "$d.CheckFileExists = $false",
      "$d.CheckPathExists = $true",
      "$d.ValidateNames = $false",
      "$d.AddExtension = $false",
      "$d.FileName = '__DSH_SELECT_CURRENT_FONT_FOLDER__'",
      "if ($d.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {",
      "  $p = $d.FileName",
      "  if ([IO.Path]::GetFileName($p) -eq '__DSH_SELECT_CURRENT_FONT_FOLDER__') { $p = [IO.Path]::GetDirectoryName($p) }",
    ...
    return new Promise((resolve, reject) => {
      ...
      child.on('exit', (code) => {
        if (code !== 0) {
          reject(new Error(stderr.trim() || ('字体来源选择器退出码 ' + code)))
          return
        }
  ...
  if (process.platform === 'darwin') {
    const script = 'POSIX path of (choose file with prompt "选择字体文件；若要挂载目录可直接填写目录路径")'
    return new Promise((resolve, reject) => {
