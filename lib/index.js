  ...
  try {
    ...
    return z.object({
      ...
      fontRoots: z
        .array(z.string())
        .default([])
        .description('外置字体来源列表：可挂载任意字体目录，也兼容单个字体文件；目录会递归扫描实际存在的字体。'),
    })
  } catch {
    return null
  }
...
/** 打开统一的字体来源选择器。
 * Windows 使用 IFileOpenDialog：
 * - 文件夹与字体文件会同时正常显示；
 * - 选中字体文件并点「挂载字体」= 挂载单个字体；
 * - 进入目标文件夹后点对话框里的「挂载当前文件夹」= 挂载整个目录。
 *
 * 不再使用 OpenFileDialog 的“伪文件名”目录选择技巧。那个方案既不能自然选择目录，
 * 也可能在关闭窗口后留下未结束的 PowerShell 进程。
 */
function pickNativeFontSource() {
...
public interface IFileDialogEvents {
  ...
  [PreserveSig] int OnOverwrite(IFileDialog pfd, IShellItem psi, out uint pResponse);
}
...
[ComImport, Guid("36116642-D713-4B97-9B83-7484A9D00433"),
 InterfaceType(ComInterfaceType.InterfaceIsIUnknown)]
public interface IFileDialogControlEvents {
  void OnItemSelected(IFileDialogCustomize pfdc, uint dwIDCtl, uint dwIDItem);
  void OnButtonClicked(IFileDialogCustomize pfdc, uint dwIDCtl);
  void OnCheckButtonToggled(IFileDialogCustomize pfdc, uint dwIDCtl, bool bChecked);
  void OnControlActivating(IFileDialogCustomize pfdc, uint dwIDCtl);
}

[ComVisible(true)]
public sealed class DshFontSourcePickerEvents : IFileDialogEvents, IFileDialogControlEvents {
  public const uint FolderButtonId = 0xD501;
  ...
  private static string PathOf(IShellItem item) {
    ...
    try {
      item.GetDisplayName(SIGDN.FILESYSPATH, out p);
  ...
  public int OnFileOk(IFileDialog pfd) {
    try {
      IShellItem item;
      pfd.GetResult(out item);
      SelectedPath = PathOf(item);
  ...
  public void OnButtonClicked(IFileDialogCustomize pfdc, uint id) {
    if (id != FolderButtonId) return;
    try {
      var pfd = (IFileDialog)pfdc;
      IShellItem folder;
      pfd.GetFolder(out folder);
      SelectedPath = PathOf(folder);
      if (!String.IsNullOrWhiteSpace(SelectedPath)) pfd.Close(0);
    } catch {}
  }
  ...
  public int OnShareViolation(IFileDialog pfd, IShellItem psi, out uint response) { response = 0; return 0; }
  public void OnTypeChange(IFileDialog pfd) {}
  public int OnOverwrite(IFileDialog pfd, IShellItem psi, out uint response) { response = 0; return 0; }
  public void OnItemSelected(IFileDialogCustomize pfdc, uint idCtl, uint idItem) {}
  public void OnCheckButtonToggled(IFileDialogCustomize pfdc, uint idCtl, bool isChecked) {}
  public void OnControlActivating(IFileDialogCustomize pfdc, uint idCtl) {}
}

public static class DshFontSourcePicker {
  ...
  public static string Pick() {
    ...
    uint cookie = 0;
    try {
      var type = Type.GetTypeFromCLSID(FileOpenDialogClsid, true);
      raw = Activator.CreateInstance(type);
      dialog = (IFileDialog)raw;
      var customize = (IFileDialogCustomize)raw;
      var events = new DshFontSourcePickerEvents();

      FOS options;
      dialog.GetOptions(out options);
      options |= FOS.FORCEFILESYSTEM | FOS.PATHMUSTEXIST | FOS.FILEMUSTEXIST | FOS.NOCHANGEDIR;
      dialog.SetOptions(options);

      dialog.SetTitle("选择字体来源");
      dialog.SetOkButtonLabel("挂载字体");
      dialog.SetFileNameLabel("字体文件");
      dialog.SetFileTypes(1, new COMDLG_FILTERSPEC[] {
        new COMDLG_FILTERSPEC {
          pszName = "字体文件 (*.ttf;*.otf;*.woff;*.woff2;*.ttc)",
          pszSpec = "*.ttf;*.otf;*.woff;*.woff2;*.ttc"
        }
      });

      customize.AddPushButton(DshFontSourcePickerEvents.FolderButtonId, "挂载当前文件夹");
      customize.MakeProminent(DshFontSourcePickerEvents.FolderButtonId);

      dialog.Advise(events, out cookie);
      int hr = dialog.Show(IntPtr.Zero);
      if (hr == unchecked((int)0x800704C7)) return null;
      if (hr < 0 && String.IsNullOrWhiteSpace(events.SelectedPath)) {
        Marshal.ThrowExceptionForHR(hr);
      }
      return events.SelectedPath;
    } finally {
      if (dialog != null && cookie != 0) {
        try { dialog.Unadvise(cookie); } catch {}
      }
      if (raw != null && Marshal.IsComObject(raw)) {
        try { Marshal.FinalReleaseComObject(raw); } catch {}
      }
    }
  }
}
`;
    ...
    const ps = [
      ...
      "$src = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('" + Buffer.from(cs, 'utf8').toString('base64') + "'))",
      "Add-Type -TypeDefinition $src -Language CSharp",
      "$p = [DshFontSourcePicker]::Pick()",
      "if (-not [string]::IsNullOrWhiteSpace($p)) { [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($p)) }",
    ].join('; ')
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        finish(reject, new Error('字体来源选择器等待超时'))
      }, 10 * 60 * 1000)
      ...
      child.on('exit', (code) => {
        if (code !== 0) {
          finish(reject, new Error(stderr.trim() || ('字体来源选择器退出码 ' + code)))
          return
        }
        const encoded = stdout.trim().split(/\r?\n/).filter(Boolean).pop() || ''
        if (!encoded) { finish(resolve, null); return }
        try {
          finish(resolve, Buffer.from(encoded, 'base64').toString('utf8'))
        } catch {
          finish(reject, new Error('无法解析字体来源路径'))
        }
      })
    })
  }
  ...
  if (process.platform === 'darwin') {
    const script = 'POSIX path of (choose file with prompt "选择字体文件；挂载目录可手动填写路径")'
    return new Promise((resolve, reject) => {
      ...
      child.on('exit', (code) => {
        ...
        else if (/User canceled/i.test(err)) resolve(null)
        else reject(new Error(err.trim() || '无法打开字体来源选择器'))
      })
    })
  }
  ...
  return new Promise((resolve, reject) => {
    const child = spawn('zenity', [
      '--file-selection',
      '--title=选择字体文件；挂载目录可手动填写路径',
      '--file-filter=字体文件 | *.ttf *.otf *.woff *.woff2 *.ttc',
    ], { stdio: ['ignore', 'pipe', 'pipe'] })
    let out = '', err = ''
    child.stdout.setEncoding('utf8'); child.stderr.setEncoding('utf8')
    child.stdout.on('data', (x) => { out += x }); child.stderr.on('data', (x) => { err += x })
...
/** 开关关闭时注入的降级说明。 */
/** 递归列出字体来源中的实际字体文件。
 * source 可以是目录，也可以是单个字体文件路径。
 */
async function walkFonts(source, prefix = '') {
  const out = []
  let st
  try {
  ...
  if (st.isFile()) {
    ...
    if (FONT_MIME[path.extname(name).toLowerCase()]) out.push(prefix || name)
    return out
  }
...
/** 扫描实际存在的字体文件。
 * DESIGN.md 只提供“已知文件名 → 友好别名”的可选映射，不再充当字体注册表：
 * - 文件不存在：不会出现在前端下拉框；
 * - 用户挂载任意目录：递归扫描到的任意字体都会自动加入；
 * - 已知字体即使目录结构与 DESIGN.md 不同，只要文件名一致仍可得到原友好别名。
 */
async function scanFonts(extraRoots, fontsRoot, aliasMap = {}) {
  const roots = [
    ...(fontsRoot ? [{ label: 'external', root: fontsRoot }] : []),
    ...extraRoots.filter(Boolean).map((r) => ({ label: 'extra', root: r })),
    { label: 'builtin', root: BUILTIN_FONTS },
  ]
  const aliasByFile = {}
  for (const [alias, rel] of Object.entries(aliasMap)) {
    const file = path.basename(String(rel || '')).toLowerCase()
    if (file && !aliasByFile[file]) aliasByFile[file] = alias
  }
  const out = []
  ...
  for (const { label, root } of roots) {
    ...
    for (const rel of rels) {
      ...
      const rawFamily = path.basename(name, ext).replace(/['"\\\r\n]+/g, ' ').trim()
      const family = aliasByFile[name.toLowerCase()] || rawFamily
      if (!family || seenFamilies.has(family)) continue
      seenFamilies.add(family)
      out.push({ rel, root: label, name, family })
    }
  }
  return out
}
...
function fontExists(rel, extraFonts, fontsRoot) {
  const roots = [...extraFonts, fontsRoot, BUILTIN_FONTS].filter(Boolean)
...
function apply(ctx) {
  ...
  // DSH Desktop 通常提供 directoryPicker native capability，可直接弹出系统文件夹选择器。
  // 这是可选能力：远程/Web 环境没有 native picker 时仍可手动填写路径。
  ...
  async function addFontsRoot(p) {
    ...
    if (!resolved) throw new Error('path required')
    let st
    try {
    ...
    if (st.isFile()) {
      ...
      if (!FONT_MIME[ext]) throw new Error('不是支持的字体文件：' + resolved)
    } else if (!st.isDirectory()) {
      throw new Error('只能挂载字体目录或字体文件：' + resolved)
    }
    if (!extraFonts.includes(resolved)) extraFonts.push(resolved)
    await persistState()
    ...
    disposers.push(
      webServer.register({
        ...
        handler: async (req, res) => {
          ...
          // 依次解析外置来源与内置字体。外置来源既可以是目录，也可以是单个字体文件。
          const roots = [...extraFonts, fontsRoot, BUILTIN_FONTS].filter(Boolean)
          for (const root of roots) {
            const resolvedRoot = path.resolve(root)
            let rootStat
            try { rootStat = await fs.stat(resolvedRoot) } catch { continue }

            let file
            if (rootStat.isFile()) {
              if (path.basename(resolvedRoot) !== path.basename(rel)) continue
              file = resolvedRoot
            } else if (rootStat.isDirectory()) {
              file = path.resolve(path.join(resolvedRoot, rel))
            ...
            if (FONT_MIME[ext] === undefined) continue
            try {
      ...
      async (endpoint, payload) => {
        ...
        switch (endpoint) {
          ...
          case 'list-fonts': {
            ...
            const fonts = await scanFonts(extraFonts, fontsRoot, aliasMap)
            // installed 仅为旧版前端兼容字段；现代 UI 直接以 fonts（实际扫描结果）为准。
            const installed = {}
            for (const a in aliasMap) installed[a] = false
            for (const f of fonts) if (Object.prototype.hasOwnProperty.call(aliasMap, f.family)) installed[f.family] = true
            return okResult({ fonts, extraFonts, aliasMap, installed })
          }
          case 'set-style':
          ...
          case 'pick-font-source':
          case 'pick-font-file': {
            try {
              const picked = await pickNativeFontSource()
              if (!picked) return okResult({ canceled: true, extraFonts: extraFonts.slice() })
              const roots = await addFontsRoot(picked)
              return okResult({ path: picked, extraFonts: roots })
            } catch (e) {
              return failResult('file-picker-failed', '无法打开字体来源选择器：' + ((e && e.message) || e))
            }
          }
          case 'pick-fonts-root': {
            if (!directoryPicker || typeof directoryPicker.capability !== 'function') {
              return failResult('picker-unavailable', '当前 DSH 没有可用的系统文件夹选择器，请手动填写路径')
            }
            const cap = directoryPicker.capability()
            if (!cap || cap.kind !== 'native' || typeof cap.pick !== 'function') {
              return failResult('picker-unavailable', '当前环境不是本机原生目录选择器，请手动填写路径')
            }
          ...
          case 'add-fonts-root':
            if (payload && typeof payload.path === 'string' && payload.path.trim()) {
              try {
                ...
                return failResult('bad-font-source', (e && e.message) || String(e))
              }
            }
