...
/** 打开系统字体文件选择器，返回绝对路径；取消时返回 null。
 * Windows 使用 WinForms OpenFileDialog，并以 UTF-8 Base64 回传路径，避免中文路径编码损坏。
 * macOS / Linux 提供轻量原生回退；不可用时抛错，由前端保留手动路径入口。
 */
function pickNativeFontFile() {
  if (process.platform === 'win32') {
    const script = [
      ...
      "if ($d.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) { [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($d.FileName)) }",
    ].join('; ')
    return new Promise((resolve, reject) => {
      ...
      child.on('exit', (code) => {
        if (code !== 0) {
          reject(new Error(stderr.trim() || ('字体文件选择器退出码 ' + code)))
          return
        }
  ...
  if (process.platform === 'darwin') {
    const script = 'POSIX path of (choose file with prompt "选择字体文件")'
    return new Promise((resolve, reject) => {
      ...
      child.on('exit', (code) => {
        ...
        else reject(new Error(err.trim() || '无法打开字体文件选择器'))
      })
    })
  }
    ...
    connection.rpc.handle(
      ...
      async (endpoint, payload) => {
        ...
        switch (endpoint) {
          ...
          case 'pick-font-file': {
            try {
              const picked = await pickNativeFontFile()
              ...
              return failResult('file-picker-failed', '无法打开字体文件选择器：' + ((e && e.message) || e))
            }
          }
